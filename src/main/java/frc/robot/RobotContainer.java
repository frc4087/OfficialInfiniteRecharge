...
public class RobotContainer {
  ...
  final public TrajectoryFollow m_trajectory = new TrajectoryFollow();
  ...
  public void teleop() {
    /*
     * if (driveJoy.getXButton()) { new Aim(); } else { if(getDriveJoy(Constants.YL)
     * > 0.5){ m_drivebase.m_drive.curvatureDrive(getDriveJoy(Constants.YL),
     * getDriveJoy(Constants.XR), isQuickTurn()); } else {
     * m_drivebase.m_drive.arcadeDrive(getDriveJoy(Constants.YL),
     * getDriveJoy(Constants.XR)); } }
     */

    /*
     * if(Math.abs(getDriveJoy(Constants.YL)) > 0.3){
     * m_drivebaseTeleop.m_drive.curvatureDrive(getDriveJoy(Constants.YL),
     * getDriveJoy(Constants.XR), isQuickTurn()); } else {
     * m_drivebaseTeleop.m_drive.arcadeDrive(getDriveJoy(Constants.YL),
     * getDriveJoy(Constants.XR)); }
     */
  }

  public void auto() throws IOException {
      m_trajectory.getAutonomousCommand();
   }
